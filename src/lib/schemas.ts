...
export const buyerSchema = z.object({
  ...
  // Store empty string as undefined so Prisma writes NULL (avoids unique collisions on empty string)
  email: z
    .string()
    .trim()
    .transform(v => (v === '' ? undefined : v))
    .optional()
    .refine(v => v === undefined || /.+@.+\..+/.test(v), { message: 'Please enter a valid email address' }),
